...
public class LambdaMain02 {
	...
	public static void main(String[] args) {
		...
		// (파라미터 선언) -> { 메서드가 할 일 구현 }
		// lambda 표현식에서는 parameter를 선언할 때 모든 type들을 생략할 수 있음.
		// 메서드 블록 안의 내용이 리턴 문장만 있는 경우에는, 중괄호({})와 return까지 생략하고 return 값만 써도 됨.
		MyMath abs2 = (double x, double y) -> {
			double result = 0;
			if (x > y) {
				result = x - y;
			} else {
