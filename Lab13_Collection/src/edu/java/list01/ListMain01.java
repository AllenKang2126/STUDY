...
public class ListMain01 {
	...
	public static void main(String[] args) {
		...
		// 생성자를 호출할 때 리스트가 저장할 수 있는 최대 개수를 지정하지 않음.
		// 변수를 선언할 때는 원소의 타입(<String>)을 반드시 써야 함. 생성자 호출에서는 원소 타입을 생략해도 됨.
		
		// 리스트가 저장하고 있는 원소의 개수
		System.out.println(list.size());
		
		// 리스트에 새로운 원소를 추가 : add(Object)
		list.add("Java");
		...
		System.out.println(list.get(list.size() - 1));  // 마지막 인덱스 = size - 1
		
		// 리스트의 특정 인덱스에 있는 원소의 값을 다른 값으로 변경 : set (int index, Object value)
		// 인덱스 2의 원소를 "Front End"로 바꿈.
		System.out.println(list.get(2));
		list.set(2, "Front end");
		System.out.println(list.get(2));
