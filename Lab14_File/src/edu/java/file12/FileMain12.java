...
public class FileMain12 {
	...
	public static void main(String[] args) {
		...
		// System.getProperty(String) : Java Runtime이 실행 중인 컴퓨터의 여러 가지 (시스템) 정보들을 리턴.
		...
		// 절대 경로(absolute path) :
		// 시스템의 루트(root, C:\, /)부터 시작해서 전체 경로 이름을 사용하는 방법.
		// (예) C:\study\lab_java\STUDY\Lab14_File\data\test.txt
		// (예) /Users/KBJ/blahblah

		// 상대 경로(relative path) :
		// 현재 작업 디렉토르(CWD)를 기준으로 파일(폴더)가 있는 위치를 찾아가는 방법.
		// (예) data\test.txt
		// (예) src/Main.java
		// . : 현재 디렉토리(폴더)
		// .. : 상위 디렉토리(폴더)
		...
		// 파일 객체를 생설할 때 절대 경로를 사용해서 생성한 경우에는, getPath 메서드는 절대경로를 리턴함.
