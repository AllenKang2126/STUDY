...
public class MethodMain05 {
	...
	public static double mean(int[] numbers) {
		int total = sum(numbers);  // 같은 클래스의 메서드를 사용해서 합계를 계산
		double avg = (double)total / numbers.length;  // 평균 계산
		return avg;
	}  // end mean()
	
	public static double mean(double[] numbers) {
		double total = sum(numbers);  // 같은 클래스의 (overloading된) 메서드를 사용해서 합계를 계산
...
//		int avg = mean(numbers);
//		for (int x : numbers) {
//		double stdDev = 0;
//			stdDev = Math.sqrt(avg - x);  // 실수함 루트값이 sqrt임.
//		}			
//		return 0.0;
//	}    - 내코드
	...
	public static double standardDeviation(int[] numbers) {
		...
		// numbers 배열의 각 원소에서 평균을 빼고 제곱한 값(x_1 - mu)^2)들을 저장할 배열을 선언.
	...
	/**
	 * 정수(int)들의 배열을 전달받아서, 그 배열의 원소들 중 최댓값을 반환하는 메서드.
	 * 
	 * @param numbers 정수(int)들의 배열
