...
public class ContactFileUtil {
	...
	public static File initializeDataDir() {
		...
		if (!dataDir.exists()) { // 데이터 폴더가 만들어져 있지 않으면0
	...
	/**
	 * 파라미터에 전달된 File 객체를 사용해서 데이터 파일에 저장된 연락처 정보를 읽어서, 연락처 정보를 List 객체로 리턴.
	 * 
...
//		return list; my code
	...
	/**
	 * 연락처 데이터를 저장하고 있는 파일(data\contacts.dat)이 있는 지 없는 지 체크해서, 데이터 파일이 있으면 파일에서
	 * ArrayList를 읽어서 리턴하고, 데이터 파일이 없으면 빈(empty) 리스트(ArrayList)를 생성해서 리턴.
	 * 
